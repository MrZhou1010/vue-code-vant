<template>
	<div class="cascader">
		...
		<div class="content">
			...
			<van-field v-model="fieldValue1" class="item-cell" label="地区" placeholder="请选择所在地区" is-link readonly
				@click="show1 = true" />
			<van-popup v-model="show1" position="bottom" round>
				<van-cascader v-model="cascaderValue1" title="请选择所在地区" :options="options" @close="show1 = false"
					@finish="onFinish1" />
			</van-popup>
			<br />
			<!-- 自定义颜色 -->
			<div style="color: #666666;">自定义颜色</div>
			<van-field v-model="fieldValue2" class="item-cell" label="地区" placeholder="请选择所在地区" is-link readonly
				@click="show2 = true" />
			<van-popup v-model="show2" position="bottom" round>
				<van-cascader v-model="cascaderValue2" title="请选择所在地区" active-color="#1989fa" :options="options"
					@close="show2 = false" @finish="onFinish2" />
			</van-popup>
			<br />
			<!-- 异步加载选项 -->
			<div style="color: #666666;">异步加载选项</div>
			<van-field v-model="fieldValue3" class="item-cell" label="地区" placeholder="请选择所在地区" is-link readonly
				@click="show3 = true" />
			<van-popup v-model="show3" position="bottom" round>
				<van-cascader v-model="cascaderValue3" title="请选择所在地区" :options="options" @close="show3 = false"
					@change="onChange" @finish="onFinish3" />
			</van-popup>
			<br />
			<!-- 自定义字段名 -->
			<div style="color: #666666;">自定义字段名</div>
			<van-field v-model="fieldValue4" class="item-cell" label="地区" placeholder="请选择所在地区" is-link readonly
				@click="show4 = true" />
			<van-popup v-model="show4" position="bottom" round>
				<van-cascader v-model="cascaderValue4" title="请选择所在地区" :show-header="false" :field-names="fieldNames"
					:options="newOptions" @close="show4 = false" @finish="onFinish4" />
			</van-popup>
		</div>
	</div>
</template>
