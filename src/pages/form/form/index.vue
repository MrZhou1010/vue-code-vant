	...
	<div class="form">
		...
		<div class="content">
			...
			<van-form @submit="onSubmit">
				...
				<van-field readonly clickable name="picker" :value="value4" label="选择器" placeholder="点击选择城市"
					@click="showPicker = true" />
				<van-popup v-model="showPicker" position="bottom" round>
					<van-picker show-toolbar :columns="columns" @confirm="onConfirm1" @cancel="showPicker = false" />
				</van-popup>
				<van-field readonly clickable name="datetimePicker" :value="value5" label="时间选择" placeholder="点击选择时间"
					@click="showDateTimePicker = true" />
				<van-popup v-model="showDateTimePicker" position="bottom" round>
					<van-datetime-picker type="datetime" @confirm="onConfirm2" @cancel="showDateTimePicker = false" />
				</van-popup>
				<van-field readonly clickable name="area" :value="value6" label="地区选择" placeholder="点击选择省市区"
					@click="showArea = true" />
				<van-popup v-model="showArea" position="bottom" round>
					<van-area :area-list="areaList" @confirm="onConfirm3" @cancel="showArea = false" />
				</van-popup>
				<van-field readonly clickable name="calendar" :value="value7" label="日历" placeholder="点击选择日期"
...
<script>
	export default {
		...
		methods: {
			...
			onConfirm3(areas) {
				this.showArea = false;
				this.value6 = areas.filter(item => !!item).map(item => item.name).join("");
			},
			onConfirm4(date) {
