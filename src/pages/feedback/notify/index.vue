<template>
	<div class="notify">
		...
		<div class="content">
			...
			<div class="content-box">
				<van-cell is-link @click="$notify({ type: 'primary', message: '通知内容' })">主要通知</van-cell>
				<van-cell is-link @click="$notify({ type: 'success', message: '通知内容' })">成功通知</van-cell>
				<van-cell is-link @click="$notify({ type: 'danger', message: '通知内容' })">危险通知</van-cell>
				<van-cell is-link @click="$notify({ type: 'warning', message: '通知内容' })">警告通知</van-cell>
			</div>
			<!-- 自定义通知 -->
			<div style="color: #666666;">自定义通知</div>
			<div class="content-box">
				<van-cell is-link @click="notify()">自定义颜色</van-cell>
				<van-cell is-link @click="$notify({ message: '自定义时长', duration: 1000 })">自定义时长</van-cell>
			</div>
			<!-- 组件调用 -->
