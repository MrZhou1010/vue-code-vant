			...
			<van-tabs>
				<van-tab v-for="(tab, index) in tabs" :key="index" :title="tab">
					<div class="tab-content" v-if="index === 0">
						<van-list v-model="isLoading1" :finished="finished1" finished-text="没有更多了" @load="onLoad1">
							<van-cell v-for="item in dataList1" :key="item" :title="item" />
						</van-list>
					</div>
					<div class="tab-content" v-if="index === 1">
						<van-list v-model="isLoading2" :error.sync="error" :finished="finished2" error-text="请求失败点击重新加载"
							finished-text="没有更多了" @load="onLoad2">
							<van-cell v-for="item in dataList2" :key="item" :title="item" />
						</van-list>
					</div>
					<div class="tab-content" v-if="index === 2">
						<van-pull-refresh v-model="isRefreshing" @refresh="onRefresh">
							<van-list v-model="isLoading3" :finished="finished3" finished-text="没有更多了" @load="onLoad3">
